...
class MPS(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, bond_dim, feature_dim=2, nCh=3,
        ...
        if adaptive_mode:
            self.linear_region = MergedLinearRegion(module_list=module_list,cudaFlag=cudaFlag,
            ...
            self.linear_region = LinearRegion(module_list=module_list,cudaFlag=cudaFlag,
                                 periodic_bc=periodic_bc,
        ...
        self.useMlp = useMlp
        if cudaFlag:
            self.linear_region = self.linear_region.cuda()
            module_list = [m.cuda() for m in module_list]
...
class LinearRegion(nn.Module):
    ...
    """
    def __init__(self, module_list, cudaFlag=False,periodic_bc=False, parallel_eval=False,
                 module_states=None):
    ...
    def forward(self, input_data):
        ...
        # contractable_list and reduce everything to get our output
            ...
            if self.cudaFlag:
                    end_vecs = [e.cuda() for e in end_vecs]
...
class MergedLinearRegion(LinearRegion):
    ...
    """
    def __init__(self, module_list, periodic_bc=False, parallel_eval=False, cudaFlag=False,
